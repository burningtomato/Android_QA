...
### 2. <span id="java_base_2">int、char、long 各占多少字节数</span>（8种基本类型）
...
StringBuilder执行效率高于StringBuffer高于String。**String是一个常量，是不可变的**，所以对于每次+=赋值都会创建一个新的对象，**StringBuilder和StringBuffer都是可变的，**当进行字符串拼接的时候采用append方法，在原来的基础上追加，所以性能要比String高，因为**StringBuffer是线程安全的而StringBuilder是线程非安全的，所以StringBuilder的效率高于StringBuffer**。对于大数据量的字符串拼接，采用StringBuffer、StringBuilder。另一种说法，**JDK1.6做了优化，通过String声明的字符串在进行用+进行拼接时，底层调用的是StringBuffer，所以性能基本上和后两者没什么区别**。
...
内部类可以很好的实现隐蔽，一般的非内部类，是不允许有private和protectd等权限的，但**内部类（除方法内部类）可以通过这些修饰符来实现隐蔽。**

内部类拥有外部类的访问权限（分静态与非静态情况），通过这一特性可以比较好的处理类之间的关联性，将一类事物的流程放在一起内部处理。【待完善】
...
抽象类是一种类别，具有构造方法。接口是一种行为规范，没有构造方法。**抽象类中可以没有抽象方法，他有自己的构造方法但是不能直接实例化对象**，所以**必须被子类继承之后才有意义。**
...
| 修饰符   | public protected default                                     | public final static                                          |
| 关键字   | extends                                                      | implements                                                   |
| 构造方法 | 具有构造方法                                                 | 没有构造方法                                                 |
|          | 只能被单继承                                                 | 可以被多实现                                                 |
| 实现     | 由默认方法实现                                               | 不存在方法的实现                                             |
| 速度     | 快                                                           | 慢，需要时间去寻找类中实现的方法                             |
| 意义     | 为子类提供一个公共的类型                                     | 是一种规范、扩展、回调                                       |
| 选择     | 有一些方法并且想让它们中的一些默认实现<br />基本功能在不断变化 | 想实现多继承<br />不断改变基本功能并且使用接口，那么就要改变所有实现了该接口的类<br />对一些类是否具有某个功能非常关心，但不关心功能的具体实现 |
...
可以，**抽象类可以没有方法和属性**，但是**含有抽象方法的类一定是抽象类**。
...
通过反射，调用add方法，例子：list.getClass().getMethod("add", Object.class).invoke(list, "abc")

参考：https://blog.51cto.com/u_9806927/3216086
...
| 属性     | Serializable                           | Parcelable                                       |
| -------- | -------------------------------------- | ------------------------------------------------ |
|          | Java的序列化接口                       | Android的序列化方式                              |
| 特点     | 使用简单，实现该接口即可，其他系统完成 | 相对复杂，实现接口的同时，需要手动实现序列化过程 |
| 内存开销 | 大                                     |                                                  |
| 存储媒介 | IO读写存储在硬盘上                     | 直接在内存中读写，内存读写速度远大于IO读写       |
| 效率     | 相对低些，会频繁引起GC                 | 高效                                             |
...
静态内部类，**只是为了降低包的深度，方便类的使用**，方便管理类结构而定义。
...
成员内部类可以无条件的访问外部类的成员，而外部类想访问成员内部类的成员却不是那么随心所欲了。

成员内部类是依附外部类而存在的，也就是说，如果要创建成员内部类的对象，前提必须存在一个外部类对象。创建成员内部类对象的一般方式如下：
...
| 属性                   | 成员内部类                   | 静态内部类           | 局部内部类                                                   | 匿名内部类                                                   |
| ---------------------- | ---------------------------- | -------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 外部类的成员           | 可以无条件访问               | 只能访问静态外部成员 | 只能访问局部final变量                                        | 只能访问局部final变量                                        |
| 是否需要依托外部类存在 | 需要                         | 不需要               | 【这怎么说，掉坑里了，<br />他的作用域只在方法内，<br />和外部类毫无关系】 | 【还在坑里】<br />作用域在调用的时候，<br />和外部类毫无关系 |
| 作用（有效）范围       | 根据修饰符                   | 根据修饰符           | 只在当前方法、作用域内                                       | 调用的地方                                                   |
| 修饰符                 | public、private 和 protected | 必须static，         | 没有                                                         | 没有                                                         |
| 外部类访问内部类       | 需要实例对象，从对象访问     | 直接访问             | 不能访问                                                     | 不能访问                                                     |

【待完善】

参考：http://c.biancheng.net/view/1031.html

参考自：[http://www.cnblogs.com/latter/p/5665015.html]【还没看完】

### 21.  <span id="java_base_21">谈谈对kotlin的理解</span>

挖个坑，以后自己填！哼哼（傲娇脸

【没找到，下面是自己胡诌的，不是答案，捂脸】

kotlin是JetBrains团队在2011年开发的语言，与Java一样，能被JVM解析，于2017年被谷歌确定为Android开发语言，同样也收到Spring、SpringBoot等服务商的支持。

相比Java，kotlin学习简单，语法和Java类似

在编程上，速度会快于Java，没有太多的冗余代码。

kotlin拥有很多现代语言的特性：

​	空安全

​	大部分都是函数式语言，移除了很多样板代码
